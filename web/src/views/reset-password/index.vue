...
const handleResetPassword = async () => {
  await authApi
    .resetPassword(props.resetToken, resetPassword.value)
    .then((res) => {
      isSuccess.value = true
      isSent.value = true
      viewMessage.value = t(`views.resetPassword.resetSuccessful`)
    })
    .catch((res) => {
      isSuccess.value = false
      isSent.value = true
      if (res.status === 404) {
        viewMessage.value = t(`views.resetPassword.tokenNotFound`)
      }else {
        console.error(res)
        viewMessage.value = t(`common.text.severError`)
      }
    })
}
</script>
    ...
    >
      <div id="card" w-320 flex-col px-20 py-35>
        <template v-if="!isSent">
          ...
          <div mt-30>
            <n-input
              v-model:value="resetPassword.newPassword"
              autofocus
